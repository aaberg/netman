...
test("Follow Up Hub page displays pending follow-ups section", async ({ page }) => {
  await page.goto("/")
  await expect(
    page.getByRole("navigation").getByRole("link", { name: "Go to Dashboard" })
  ).toBeVisible()
  ...
  const pendingFollowUpsSection = page
    .getByText("No pending follow-ups found")
    .or(page.getByRole("heading", { name: "Pending Follow-ups" }))
