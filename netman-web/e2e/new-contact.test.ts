...
test("can register a new contact", async ({page}) => {
    ...
    await page.locator("li:has-text('Email address')").locator("input[placeholder='Label']").fill("Work")
    
    await page.getByText("+ Add phone").click()
    await page.getByPlaceholder("Phone number").fill("12345678")
    await page.locator("li:has-text('Phone number')").locator("input[placeholder='Label']").fill("Mobile")
